# H.py - COMPLETE FIXED VERSION FOR RENDER WITH TERMINAL FEATURE
import telebot
import subprocess
import os
import zipfile
...
@app.route('/')
def home():
    return "🤖 Bot is running on Render with Terminal Feature!"
...
terminal_sessions = {}
TERMINAL_TIMEOUT = 1800  # 30 minutes timeout
...
def create_terminal_session(user_id, chat_id):
    """Create a new terminal session with PTY"""
    try:
        # Use bash shell with PTY
        master_fd, slave_fd = pty.openpty()
        
        process = subprocess.Popen(
            ['/bin/bash', '--norc', '--noediting'],
            stdin=slave_fd,
            stdout=slave_fd,
            stderr=slave_fd,
        ...
        output_thread = threading.Thread(target=read_terminal_output, args=(user_id,))
        output_thread.daemon = True
        output_thread.start()
        
        logger.info(f"Terminal session created for user {user_id}")
        return True
    except Exception as e:
        logger.error(f"Error creating terminal session: {e}")
        return False

def read_terminal_output(user_id):
    """Read terminal output and send to user"""
    session = terminal_sessions.get(user_id)
    if not session:
        return
    ...
    while user_id in terminal_sessions:
        try:
            # Use select to check for output
            ...
            if time.time() - session['last_activity'] > TERMINAL_TIMEOUT:
                logger.info(f"Terminal session timeout for user {user_id}")
                break
                
        except Exception as e:
            logger.error(f"Error in terminal output reader: {e}")
            break
    
    # Cleanup on exit
    cleanup_terminal_session(user_id)

def cleanup_terminal_session(user_id):
    """Clean up terminal session resources"""
    session = terminal_sessions.pop(user_id, None)
    if session:
        try:
            ...
            logger.info(f"Terminal session cleaned up for user {user_id}")
        except Exception as e:
            logger.error(f"Error cleaning terminal session: {e}")

def execute_terminal_command(user_id, command):
    """Execute a command in the terminal session"""
    session = terminal_sessions.get(user_id)
    if not session:
        return create_terminal_session_for_user(user_id, command)
    ...
    try:
        ...
        if command.startswith('cd '):
            ...
            try:
                # Handle relative paths with spaces
                ...
                # Change directory
                os.chdir(new_dir if os.path.isabs(new_dir) else 
                        os.path.join(session.get('current_dir', os.getcwd()), new_dir))
                session['current_dir'] = os.getcwd()
                output = f"Directory changed to: {session['current_dir']}\n"
                bot.send_message(session['chat_id'], f"```bash\n{output}\n```", parse_mode='Markdown')
            except Exception as e:
                bot.send_message(session['chat_id'], f"Error: {str(e)}")
            return
        ...
        if output:
            ...
            bot.send_message(session['chat_id'], "Command executed (no output)")
            
    except Exception as e:
        bot.send_message(session['chat_id'], f"Terminal error: {str(e)}")
        logger.error(f"Error executing terminal command: {e}")

def create_terminal_session_for_user(user_id, initial_command=None):
    """Create a fresh terminal session for user"""
    # Cleanup old session if exists
    if user_id in terminal_sessions:
        cleanup_terminal_session(user_id)
    
    # Create new session
    result = create_terminal_session(user_id, None)  # Will be updated
    if result:
        # Update chat_id
        session = terminal_sessions.get(user_id)
        if session:
            # Get the latest message's chat_id from the bot's context
            # This is handled in the command handler
            return True
    return False

def send_terminal_help(chat_id):
    help_text = (
        "💻 **Terminal Help**\n\n"
        "Commands:\n"
        "- `/terminal` - Start a new terminal session\n"
        "- `/term <command>` - Execute single command\n"
        "- `/termkill` - Kill current terminal session\n"
        "- `/termdir <path>` - Change working directory\n\n"
        "**Auto-install Features:**\n"
        "- Git will be auto-installed if needed\n"
        "- Python packages auto-install via pip\n"
        "- Node modules auto-install via npm\n\n"
        "**Available Commands:**\n"
        "`ls`, `pwd`, `cd`, `cat`, `echo`, `mkdir`, `rm`, `cp`, `mv`\n"
        "`git clone`, `python`, `node`, `pip install`, `npm install`\n"
        "`apt-get install` (requires permission)\n\n"
        "Session auto-expires after 30 minutes of inactivity."
    )
    bot.send_message(chat_id, help_text, parse_mode='Markdown')

def install_package_if_needed(package_name, chat_id):
    """Auto-install packages if required"""
    try:
        ...
        # Try to install
        bot.send_message(chat_id, f"🔧 Installing `{package_name}` automatically...")
        ...
        if install_result.returncode == 0:
            bot.send_message(chat_id, f"✅ `{package_name}` installed successfully!")
            return True
        else:
            bot.send_message(chat_id, f"❌ Failed to install `{package_name}`")
            return False
    except Exception as e:
        logger.error(f"Error installing package: {e}")
        bot.send_message(chat_id, f"Error installing package: {str(e)}")
        return False
...
def cmd_terminal(message):
    ...
    if bot_locked and user_id not in admin_ids:
        bot.reply_to(message, "Bot is locked. Terminal unavailable.")
        return
    
    # Create/Recreate terminal session
    if user_id in terminal_sessions:
        cleanup_terminal_session(user_id)
    
    if create_terminal_session(user_id, chat_id):
        session = terminal_sessions.get(user_id)
        if session:
            session['chat_id'] = chat_id
        bot.reply_to(message, 
            "💻 **Terminal Started!**\n\n"
            "Use `/term <command>` to execute commands.\n"
            "Example: `/term ls -la`\n\n"
            "Commands: `/termkill` to end session, `/termhelp` for help",
            parse_mode='Markdown'
        )
        # Show current directory
        bot.send_message(chat_id, f"📁 Current directory: `{os.getcwd()}`", parse_mode='Markdown')
    else:
        bot.reply_to(message, "❌ Failed to start terminal session")

@bot.message_handler(commands=['term'])
def cmd_term(message):
    user_id = message.from_user.id
    chat_id = message.chat.id
    ...
    if not message.text or len(message.text.split()) < 2:
        bot.reply_to(message, "Usage: `/term <command>`\nExample: `/term ls -la`", parse_mode='Markdown')
        return
    ...
    if command.startswith('git clone'):
        if not install_package_if_needed('git', chat_id):
            return
    ...
    if user_id not in terminal_sessions:
        ...
        if create_terminal_session(user_id, chat_id):
            ...
            if session:
                session['chat_id'] = chat_id
            bot.send_message(chat_id, "💻 Auto-created terminal session")
        else:
            bot.reply_to(message, "❌ Failed to create terminal session. Try `/terminal` first")
            return
    
    execute_terminal_command(user_id, command)

@bot.message_handler(commands=['termkill'])
def cmd_termkill(message):
    user_id = message.from_user.id
    chat_id = message.chat.id
    ...
    if user_id in terminal_sessions:
        ...
        bot.reply_to(message, "No active terminal session")

@bot.message_handler(commands=['termhelp'])
def cmd_termhelp(message):
    send_terminal_help(message.chat.id)

@bot.message_handler(commands=['termdir'])
def cmd_termdir(message):
    user_id = message.from_user.id
    chat_id = message.chat.id
    
    if not message.text or len(message.text.split()) < 2:
        bot.reply_to(message, "Usage: `/termdir <path>`", parse_mode='Markdown')
        return
    ...
    try:
        os.chdir(new_dir)
        bot.reply_to(message, f"Directory changed to: `{os.getcwd()}`", parse_mode='Markdown')
        
        # Update session directory if exists
        if user_id in terminal_sessions:
            terminal_sessions[user_id]['current_dir'] = os.getcwd()
    except Exception as e:
...
def cmd_commands(message):
    ...
    commands_list = (
        ...
        "**File Management:**\n"
        "/uploadfile - Upload script file\n"
        "/checkfiles - List your files\n"
        "/runningallcode - Run all approved scripts (Admin)\n\n"
        
        "**Terminal Commands:**\n"
        "/terminal - Start terminal session\n"
        "/term <command> - Execute command\n"
        "/termkill - Kill terminal session\n"
        "/termhelp - Terminal help\n"
        "/termdir <path> - Change directory\n\n"
        
        "**Support:**\n"
        "/mpx <query> - AI assistant\n"
        "/contactowner - Contact bot owner\n"
        "/updateschannel - Get update channel\n\n"
        ...
        "/pending - View pending files\n"
        "/statistics - View detailed stats\n"
    )
    
    bot.reply_to(message, commands_list, parse_mode='Markdown')
...
def create_main_menu_inline(user_id):
    ...
    buttons = [
        ...
        types.InlineKeyboardButton('💻 Terminal', callback_data='terminal_ai'),
...
def _logic_terminal(message):
    ...
    if create_terminal_session(user_id, chat_id):
        ...
        bot.reply_to(message, 
            ...
            "Use `/term <command>` to execute commands.\n"
            "Example: `/term ls -la`\n"
            "Use `/termkill` to end session.",
            parse_mode='Markdown'
        )
    else:
        bot.reply_to(message, "❌ Failed to start terminal")

def _logic_commands(message):
    cmd_commands(message)
...
def _logic_send_welcome(message):
    ...
    welcome_msg_text = (f"Welcome, {user_name}!\n\nYour User ID: `{user_id}`\n"
                        ...
                        f"Use `/terminal` for shell access!\n\n"
...
def handle_pending_command(message): _logic_view_pending(message)
...
BUTTON_TEXT_TO_LOGIC = {
    ...
    "💻 Terminal": _logic_terminal,
    "📋 Commands": _logic_commands
}
...
def handle_callbacks(call):
    ...
    if bot_locked and user_id not in admin_ids and data not in ['back_to_main', 'speed', 'stats', 'mpx_ai', 'uptime', 'terminal_ai', 'show_commands']:
    ...
    try:
        ...
        elif data == 'terminal_ai':
            bot.answer_callback_query(call.id)
            _logic_terminal(call.message)
        elif data == 'show_commands':
            bot.answer_callback_query(call.id)
            cmd_commands(call.message)
        elif data == 'uptime':
            bot.answer_callback_query(call.id)
            uptime_str = get_uptime()
...
def back_to_main_callback(call):
    ...
    main_menu_text = (f"Welcome back, {call.from_user.first_name}!\n\nID: `{user_id}`\n"
                      ...
                      f"Use buttons or type commands.\n"
                      f"💻 Try `/terminal` for shell access!")
    try:
        bot.answer_callback_query(call.id)
...
def cleanup():
    ...
    for user_id in list(terminal_sessions.keys()):
        cleanup_terminal_session(user_id)
...
if __name__ == '__main__':
    ...
    logger.info("="*40 + "\nBot Starting Up on Render with Terminal Support...\n" + f"Python: {sys.version.split()[0]}\n" +
